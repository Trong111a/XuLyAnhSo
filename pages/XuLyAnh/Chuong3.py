...
def HistEqualColor(imgin):
    ...
    if len(imgin.shape) == 2:  
        imgin = cv2.cvtColor(imgin, cv2.COLOR_GRAY2BGR)  # Chuyển thành ảnh màu 3 kênh
...
def MyBoxFilter(imgin):
    M, N = imgin.shape  # Kích thước ảnh (M: số hàng, N: số cột)
    imgout = np.zeros((M, N), np.uint8)
    
    m, n = 11, 11  # Kích thước bộ lọc
    w = np.ones((m, n)) / (m * n)  # Bộ lọc trung bình

    a, b = m // 2, n // 2  # Kích thước biên
